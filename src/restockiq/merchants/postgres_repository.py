...
class PostgresMerchantRepository(MerchantRepository):
    ...
    async def save(self, merchant: Merchant) -> None:  # type: ignore[override]
    ...
    async def get_by_id(self, merchant_id: MerchantId) -> Merchant:  # type: ignore[override]
    ...
    async def list_all(self) -> Sequence[Merchant]:  # type: ignore[override]
        result = await self._session.execute(select(MerchantModel))
        return [self._to_domain(m) for m in result.scalars().all()]

    async def exists(self, merchant_id: MerchantId) -> bool:  # type: ignore[override]
